...
def main(cfg: DictConfig):
    ...
    for _ in range(num_restarts):
        ...
        idx = random.choice(rng, num_samples, (num_samples,), replace=False)
        var, ls = gp_mle_sgd(s_obs[idx], f_obs[idx], matern_1_2)
        print(f"var {var:0.4f}, ls {ls:0.4f}")
        thetas += [[var, ls]]
    var_mu, ls_mu = jnp.array(thetas).mean(axis=0)
    print(f"Average: var {var_mu:0.4f}, ls {ls_mu:0.4f}")
