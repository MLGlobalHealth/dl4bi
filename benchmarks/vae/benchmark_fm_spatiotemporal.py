#!/usr/bin/env python3
"""benchmark_fm_spatiotemporal.py
...
def make_priors() -> dict:
    ...
    return {
        ...
        "nu":    dist.Uniform(2, 4),       # matches original spatiotemporal_kernel.py
        "beta":  dist.Normal(),
    }
...
def gen_train_dataloader(st: Array, priors: dict, batch_size: int = BATCH_SIZE):
    ...
    def dataloader(rng_data):
        while True:
            rng_data, rng_ls, rng_a, rng_al, rng_nu, rng_z = random.split(rng_data, 6)
            ls    = priors["ls"].sample(rng_ls)
            a     = priors["a"].sample(rng_a)
            alpha = priors["alpha"].sample(rng_al)
            nu    = priors["nu"].sample(rng_nu)
            K = gneiting_kernel(s_all, t_all, s_all, t_all, 1.0, ls, a, alpha, 1.0, nu)
...
def reload_state(ckpt_dir: Path, model: nn.Module, st: Array, optimizer) -> TrainState:
    ...
    dummy = {
        "s": st,
        "z": jnp.ones((1, TL)),
        "conditionals": jnp.array([1.0, 0.5, 0.5, 2.0]),
        "f": jnp.ones((1, TL)),
    }
