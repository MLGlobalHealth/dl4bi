...
def main(gender="female", seed=42):
    wandb.init(mode="disabled")
    rng = random.key(seed)
    rng_train, rng_test, rng_infer = random.split(rng, 3)
    save_dir = Path(f"results/cancer_mortality_{gender}/")
    save_dir.mkdir(parents=True, exist_ok=True)
    map_data = gpd.read_file(
        f"benchmarks/vae/maps/{gender}_under_50_cancer_mortality_LAD_2023"
    )
    ...
    population = (
        jnp.array(map_data.population, dtype=jnp.int32) // 100
