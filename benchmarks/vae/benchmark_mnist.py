...
Setup:
  ...
  - Target : normalised MNIST patch
  - Loss   : OT-CFM for FM-DeepRV, MSE for DeepRV
  - Inference: observe 30% of pixels, Gaussian likelihood, HMC inpaints the rest

Models:
  - DeepRV + gMLP         (single forward pass, trained independently)
  - FM-DeepRV K=1,3,5     (trained once, evaluated with shared weights at decode time)

Run from the repo root:
    uv run python benchmarks/vae/benchmark_mnist.py
"""
    ...
    def dataloader(rng_data):
        while True:
            ...
            yield {
                "s": s,
                "z": z,
                "conditionals": jnp.array([0.0]),  # no hyperparameter conditioning
                "f": f,
            }
...
def build_inpainting_model(s: Array) -> Callable:
    """
    HMC model: z ~ N(0,I), f = surrogate(z), y_obs ~ N(f, sigma) at observed pixels.

    obs_mask and y are passed as runtime arguments so numpyro does not
    recompile the model per image.  y must be full-length [L] with zeros
    (or any value) at unobserved positions — the mask handles exclusion.
    """
...
def surrogate_model_train(
    ...
    """Train a surrogate model and return (train_time, eval_mse, state, ...)."""
...
def run_hmc_inpaint(
    ...
    """
    y_obs : full-length [L] array — zeros at unobserved positions.
    obs_mask : boolean [L] — True where observed.
    """
...
def main(seed: int = 42):
    rng = random.key(seed)
    save_dir = Path("results/mnist_benchmark/").resolve()
    ...
    # FM-DeepRV: train once with n_steps=1 (K only affects decode, not training).
    # DeepRV: trained independently.
    train_configs = {
        "DeepRV + gMLP": (
    ...
    # DeepRV: one decoder.
    # FM-DeepRV: one shared set of weights, one decoder per K.
    ...
    inpaint_model = build_inpainting_model(s)

    # --- Inference on N_TEST test images ---
    ...
    for img_i in range(N_TEST):
        ...
        # Full-length observation: noise at observed pixels, zero elsewhere.
        # The model mask excludes unobserved positions from the likelihood.
        y_obs = jnp.where(
            obs_mask,
