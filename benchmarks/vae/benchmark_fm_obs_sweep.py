...
TIME_STEPS = 5                 # T
T_OBS_MASK = jnp.array([True, True, True, True, True])    # FIXED: full time obs
OBS_RATIO = 0.5                # spatial sites observed per time step
...
def main(seed: int = 19, obs_ratio: float = OBS_RATIO):
    rng = random.key(seed)
    save_dir = Path("results/fm_spatiotemporal/").resolve()
    save_dir.mkdir(parents=True, exist_ok=True)
    results_dir = save_dir / f"obs_{obs_ratio:.2f}"
    results_dir.mkdir(parents=True, exist_ok=True)
    print(f"obs_ratio={obs_ratio:.2f}  hmc warmup={HMC_WARMUP} samples={HMC_SAMPLES}")
    ...
    spat_mask = gen_obs_mask(rng_mask, obs_ratio=obs_ratio)             # [L]  spatial obs mask
    ...
    plot_predictive_means(
        ...
        results_dir / "predictive_means.png",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hmc-warmup", type=int, default=HMC_WARMUP)
    parser.add_argument("--hmc-samples", type=int, default=HMC_SAMPLES)
    args = parser.parse_args()
    # Override module-level HMC budgets so run_hmc picks them up.
    sys.modules[__name__].HMC_WARMUP = args.hmc_warmup
    sys.modules[__name__].HMC_SAMPLES = args.hmc_samples
    main(seed=args.seed, obs_ratio=args.obs_ratio)
