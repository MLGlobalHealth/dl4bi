...
def main(s, mcmc_path, batch_size=8, res=150, thinning=1):
    rng = random.key(s)
    ...
    print(
        "Shapes of s_c, y_c, s_t, x_t:",
        s_c.shape,
        y_c.shape,
        s_t.shape,
        x_t.shape if x_t is not None else None,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--thinning", type=int, default=1, help="Thinning factor."
    )
    parser.add_argument("-s", "--seed", type=int, default=11, help="Random seed.")
    args = parser.parse_args()

    main(args.seed, args.mcmc_path, args.batch_size, thinning=args.thinning)
